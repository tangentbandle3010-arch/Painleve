...
\begin{proposition}\label{prop:2.1_wronskian-ode}
...
したがって、$p_{1}(x)$ は厳密に
\begin{equation}\label{eq:2.4_p1_coefficient_1-ord_pole}
...
%#endregion
%#region
これまでの議論を統合し、リーマン・ヒルベルト問題を以下のように再定式化する。

\begin{itemize}
    \item[] \textbf{リーマン・ヒルベルト問題（現代的形式）} \\
    $(\mathbb{P}^{1}(\mathbb{C}), \Xi, \hat{\rho})$ を本質的なリーマンデータとする。このとき、ある適切な有限集合 $\Lambda \subset \mathbb{P}^{1}(\mathbb{C})$ （みかけの特異点）を導入し、その拡大 $(\mathbb{P}^{1}(\mathbb{C}), \Xi \cup \Lambda, \hat{\rho'})$ をリーマンデータとして持つような $n$ 階フックス型微分方程式を構成できるか？
\end{itemize}

この問題に対して、現代の微分方程式論では以下の強力な結論が導かれている。

\begin{proposition}\label{prop:riemann_existence_theorem}
    上記のリーマン・ヒルベルト問題は、みかけの特異点の数 $g$ を次元の不足分 $K(n,m)$ 以上に取ることで、任意のリーマンデータに対して常に肯定的に解決される。
\end{proposition}

これにより、我々は「モノドロミーを固定したまま特異点を動かす」という等モノドロミー変形の議論において、常にその背後に対応する微分方程式が存在することを確信して議論を進めることが可能となる。
%#endregion
%#region
これまでの議論はリーマン球面 $\mathbb{P}^{1}(\mathbb{C})$ 上で行ってきたが、一般のリーマン面 $\mathbf{R}$ 上に拡張してリーマン・ヒルベルトの問題を考えることができる。
なぜなら、微分方程式のある点が「確定特異点であるか否か」という性質は、局所座標を用いたべき級数展開の振る舞い（高々多項式オーダーの増大度）によって判定される局所的な性質であり、局所座標の取り方（正則な座標変換）に依存しないからである。

したがって、一般のリーマン面 $\mathbf{R}$ 上のフックス型微分方程式に対しても、リーマンデータ
\[
(\mathbf{R}, \Xi, \hat{\rho})
\]
を矛盾なく定義することができる。ここで、$\Xi$ は $\mathbf{R}$ 上の離散部分集合（特異点の集合）であり、$\hat{\rho}$ は特異点を除いた空間の基本群からの表現
\[
\rho : \pi_{1}(\mathbf{R} \setminus \Xi) \to GL(n, \mathbb{C})
\]
の同値類である。$n$は微分方程式の階数である。

\begin{Q}[【核心】リーマン面 $\mathbf{R}$ 上に拡張したとき、表現 $\rho$ の定義域が $\pi_{1}(\mathbf{R})$ ではなく $\pi_{1}(\mathbf{R} \setminus \Xi)$ にならなければならない理由は何か？]
\textbf{A.} モノドロミー（解の解析接続）は、「特異点を避けて」回ることで初めて意味を持つからである。

もし特異点を含めた空間全体 $\mathbf{R}$ の基本群 $\pi_{1}(\mathbf{R})$ を考えてしまうと、ループが特異点 $\xi_j$ の上を通過してしまったり、特異点を囲むループが特異点そのものを通過して1点に連続的に縮められてしまう（自明なループになる）ことが許容される。これでは特異点周りの解の分岐（モノドロミー行列）を測ることができなくなってしまう。
そのため、必ず特異点集合 $\Xi$ を障害物としてくり抜いた空間 $\mathbf{R} \setminus \Xi$ の基本群を考える必要がある。これは $\mathbb{P}^{1}(\mathbb{C})$ のときに $\mathbb{X} = \mathbb{P}^{1}(\mathbb{C}) \setminus \Xi$ と設定したことと全く同じ論理である。
\end{Q}

\begin{Q}[【基礎】「複素平面の貼り合わせ」で定義されるリーマン面 $\mathbf{R}$ とは、厳密には何なのか？「複素多様体」と同じものか？]
\textbf{A.} 結論から言えば、リーマン面とは\textbf{「複素1次元の複素多様体」}のことである。
厳密な定義は定義\ref{def:riemann_surface}参照。\\
この「正則関数で滑らかに貼り合わせる」という性質により、$\mathbf{R}$ 全体で「正則性」や「有理型関数の極の位数」といった解析的な概念が、どの局所座標系を選んでも矛盾なく（well-definedに）扱えるようになるのである。我々が扱ってきたリーマン球面 $\mathbb{P}^1(\mathbb{C})$ は、最も基本的でコンパクトなリーマン面（種数0）の代表例である。
\end{Q}

以下、リーマン面 $\mathbf{R}$ として「コンパクトなリーマン面（閉リーマン面）」を考える。（※みかけの特異点の個数 $g$ との記号の衝突を避けるため、以下では $\mathbf{R}$ の種数を $p$ と表す。）
本来の確定特異点の集合を $\Xi$ とし、$|\Xi| = m+1$ とする。

\begin{Q}[【核心】一般のリーマン面への拡張を考える際、なぜ「閉リーマン面（コンパクト）」に限定しなければならないのか？開リーマン面（例えば複素平面 $\mathbb{C}$ 全体）では何が問題なのか？]
\textbf{A.} コンパクトでないと、微分方程式の係数（有理型関数）の持つ自由度（パラメータの数）が無限次元に爆発してしまい、数え上げという行為自体が破綻するからである。

複素解析の基本定理（リウヴィルの定理など）により、コンパクトなリーマン面上では「極を持たない正則な関数は定数しかない」という強い制約が働く。この制約があるおかげで、極の位置と位数を指定したときの有理型関数の空間は「有限次元のベクトル空間（リーマン・ロッホの定理で計算可能）」に収まる。
一方、開リーマン面（例えば $\mathbb{C}$）上には、$e^x, \sin x$ のような極を持たない正則関数（整関数）が無数に存在し、パラメータの自由度は無限大となる。リーマン・ヒルベルト問題のように「次元（パラメータ数）を比較して方程式の存在を論じる」理論においては、舞台がコンパクトなリーマン面であることが絶対条件なのである。
\end{Q}

閉リーマン面 $\mathbf{R}$ 上であっても、特異点近傍での解の振る舞い（確定特異点であることや、特性指数の定義）は局所座標の表示のみに依存するため、球面の場合と全く同様に議論できる。
したがって、特性指数に関する自由度 $E_{0}^{(p)}(n,m)$ は種数 $p$ には依存せず、
\[
E_{0}^{(p)}(n,m) = (m+1)n - 1 = E_{0}(n,m)
\]
となる。

また、$\mathbf{R}$ 上の $n$ 階フックス型微分方程式の係数パラメータの総数 $E^{(p)}(n,m)$ や、モノドロミー表現の自由度 $M^{(p)}(n,m)$ も計算することができ、これらは以下の式で与えられることが知られている。
\begin{align}
E^{(p)}(n,m) &= \frac{1}{2}n^{2}(m+1+2p-2) + \frac{1}{2}n(m+1)\\
M^{(p)}(n,m) &= n^{2}(m+1+2p-2) + 1
\end{align}
これらの式において種数 $p=0$ （リーマン球面）とすると、これまでに導出した $E(n,m)$ および $M(n,m)$ の式と完全に一致する。

\begin{Q}[【基礎】モノドロミーの自由度 $M^{(p)}(n,m)$ の式に含まれる「$2p$」という項はどこから現れたのか？]
\textbf{A.} 種数 $p$ の閉リーマン面が持つ「ハンドル（穴）」を通る、基本群の生成元の数に由来する。

特異点集合 $\Xi$ を取り除いた空間 $\mathbf{R} \setminus \Xi$ の基本群 $\pi_1$ は、特異点を回るループ $\gamma_1, \dots, \gamma_{m+1}$ の他に、各ハンドルを通る「縦のループ $a_k$」と「横のループ $b_k$」のペアが $p$ 個（計 $2p$ 個）存在し、これらがすべて生成元となる。
全体の大域的な関係式は1つ（特異点のループとハンドルの交換子の積が自明になる）だけであるため、自由にモノドロミー行列を割り当てられる生成元は
\[
(m+1) + 2p - 1 = m + 2p
\]
個となる。各行列は $GL(n, \mathbb{C})$ の元なので $n^2$ 個のパラメータを持ち、最後に基底の取り替え（相似変換）の自由度 $n^2 - 1$ を引くことで、
\[
(m+2p)n^2 - (n^2 - 1) = n^2(m+2p-1) + 1 = n^2(m+1+2p-2) + 1
\]
となり、$M^{(p)}(n,m)$ の式が代数的に完全に導出される。
\end{Q}

最後に、次元の不足を補うためにみかけの特異点を $g$ 個追加する場合を考える。
確定特異点がみかけの特異点であるかどうか（特性指数が整数差で対数項がないこと）は完全に局所的な条件であるため、大域的な形状（種数 $p$）には依存しない。
したがって、みかけの特異点がもたらすパラメータ（特異点の位置座標の自由度）は球面の場合と同様に1つにつき1増えるため、微分方程式の全パラメータ数は
\[
E^{(p)}(n,m) + g
\]
となる。
...
%#endregion
%#region
これまでの議論を一般のコンパクトリーマン面 $\mathbf{R}$ （種数 $p$）へと拡張する。
命題 \ref{prop:riemann_existence_theorem} により、十分な個数のみかけの特異点を許容すればリーマンの問題は常に解けるが、その「最小個数」について以下の重要な事実が知られている。

\begin{proposition}\label{prop:minimal_singularities}
    リーマンデータ $(\mathbf{R}, \Xi, \hat{\rho})$ における表現類 $\hat{\rho}$ が既約であるならば、高々
    \[
    K^{(p)}(n,m) = M^{(p)}(n,m) - E^{(p)}(n,m)
    \]
    個のみかけの特異点を追加した $n$ 階フックス型微分方程式が存在して、そのモノドロミーは $\hat{\rho}$ と一致する。
\end{proposition}

この $K^{(p)}(n,m)$ は、連立系（シュレージンガー型など）が持つ豊かなパラメータ空間の次元と、単独 $n$ 階方程式が持つ比較的制約の強いパラメータ空間の次元の「差」を、みかけの特異点の座標という形で完全に補償するものである。
既約性という条件は、この次元の補償が「最も効率的に（最小個数で）」行われるための代数的な保証となっている。

ここで特筆すべきは、リーマン球面 ($p=0$) において $n=2$ 次の場合、$K^{(0)}(2,m) = m-2$ となる点である。特異点が4つの場合 ($m=3$)、我々はシュレージンガー型からの単独高階化によって $g=1$ 個のみかけの特異点を導出したが、これは $3-2=1$ と見事に一致する。
この $g=1$ 個のみかけの特異点 $\lambda$ の挙動を追うことが、次節のパンルヴェ方程式の理解へと直結する。
%#endregion

\subsection*{ガウスの超幾何では「救えない」ケース：可約な表現とみかけの特異点}
%#region
特異点が3点 $\{0,1,\infty\}$ であっても、モノドロミーが可約な場合には、ガウスの超幾何微分方程式（単独2階）では実現できない表現が存在する。その典型例が、以下の対角表現である。
\[
\Gamma(\gamma_{0})=
\begin{pmatrix}
1&0\\
...
\begin{Q}[【核心】なぜこの表現は、特異点3点の単独2階方程式として存在できないのか？]
\textbf{A.} 単独2階フックス型微分方程式が満たすべき「フックスの関係式」を破ってしまうからである。

特異点が3点 ($x=0,1,\infty$) の2階単独方程式において、各点での特性指数の和の総計は必ず $1$ にならなければならない。しかし、上記の表現に対応する指数を $\theta_k = \frac{\log k}{2\pi i}$ とおいて計算すると、
\begin{itemize}
    \item $x=0$: $\{0, \theta_\alpha\}$ \quad (和 $\theta_\alpha$)
    \item $x=1$: $\{0, \theta_\beta\}$ \quad (和 $\theta_\beta$)
    \item $x=\infty$: $\{0, -\theta_\alpha - \theta_\beta\}$ \quad (和 $-\theta_\alpha - \theta_\beta$)
\end{itemize}
となり、指数の総和は $\theta_\alpha + \theta_\beta - \theta_\alpha - \theta_\beta = 0$ となる。
これはフックスの関係式 $\sum \Sigma_j = 1$ に矛盾するため、このモノドロミーを持つ単独2階方程式は、みかけの特異点なしには構成不可能である。
\end{Q}

この「指数の不足」を解消するために、みかけの特異点 $x=\lambda$ を導入する。$x=\lambda$ での特性指数を $\{0, 2\}$ とすれば、その点の指数の和 $2$ が加わり、全体の指数和は $0 + 2 = 2$ となる。これは特異点4点の場合のフックスの関係式の値 $\frac{2(1)}{2}(4-2) = 2$ と一致する。
したがって、この可約な表現は、\textbf{「1つのみかけの特異点を持つ特異点4点の方程式」}として初めて実現されるのである。
%#endregion

\subsection{フックスの問題}

%#region
リーマン球面 $\mathbb{P}^{1}(\mathbb{C})$ 上に定義された $n$ 階フックス型微分方程式\eqref{eq:linear-ode-n}に対し、以下の性質 (P1)--(P3) を仮定する。

\begin{itemize}
    \item[(P1)] $x=\xi_{1}, \dots, \xi_{m}, \xi_{m+1}=\infty$ を確定特異点、$x=\lambda_{1}, \dots, \lambda_{g}$ をみかけの特異点としてもつ。
    \item[(P2)] 各 $x=\xi_{i}$ における特性指数のどの2つも、その差が整数ではない。
    \item[(P3)] モノドロミー表現 $\hat{\rho}$ は既約である。
\end{itemize}

本質的な確定特異点の集合を $\Xi = \{\xi_{1}, \dots, \xi_{m+1}\}$、みかけの特異点の集合を $\Lambda = \{\lambda_{1}, \dots, \lambda_{g}\}$ とすると、命題\ref{prop:minimal_singularities}と(P3)からみかけの特異点の数は
\[
K(n,m)=M(n,m)-E(n,m)
\]
に等しいとしてよい。方程式 \eqref{eq:linear-ode-n} が持つリーマンデータは
\[
(\mathbb{P}^{1}(\mathbb{C}), \Xi \cup \Lambda, \hat{\rho})
\]
であり、これは本質的なリーマンデータ $(\mathbb{P}^{1}(\mathbb{C}), \Xi, \hat{\rho}_{0})$ の拡大となっている。

\begin{Q}[【核心】なぜこれら (P1)--(P3) の仮定が必要なのか？]
\textbf{A.} 等モノドロミー変形（パンルヴェ方程式の導出）を行うための「最もジェネリック（一般的）で扱いやすい舞台」を設定するためである。

(P2) によって解から対数項を排除し、(P3) によって方程式がより低階の方程式に分解される可能性を排除している。このクリーンな設定の下で、特異点 $\Xi$ の位置を動かしたときに、モノドロミー $\hat{\rho}$ を不変に保つように係数およびみかけの特異点 $\Lambda$ がどのように連動して動くべきかを記述することで、パンルヴェ方程式をハミルトン系の形で導出することが可能となる。
\end{Q}
%#endregion
%#region
次に、\eqref{eq:linear-ode-n} がある領域 $U \subset \mathbb{C}^{L}$ でパラメータ $t=(t_{1},\cdots,t_{L})\in U$ に解析的に依存するとする。つまり、各点 $x$ で $t$ について解析的であるような係数 $p_{1}(x,t),\cdots,p_{n}(x,t)$ をもつ以下のような微分方程式の族を考える。
\begin{equation}\label{eq:monodromy_deformation}
\frac{d^{n}y}{dx^{n}}+\sum_{j=1}^{n} p_{j}(x,t)\frac{d^{n-j}y}{dx^{n-j}} = 0   
\end{equation}
ここで、ある基準となる時刻 $t_{0}\in U$ において $p_{j}(x,t_{0})=p_{j}(x)$ となり、元の方程式に一致するとする。
\eqref{eq:monodromy_deformation} のリーマンデータを、
\[
(\mathbb{P}^{1}(\mathbb{C}),\Xi(t)\cup\Lambda(t),\hat{\rho})
\]
とする。ここで、本来の確定特異点の集合を $\Xi(t)=\{\xi_{1}(t),\cdots,\xi_{m}(t),\xi_{m+1}(t)=\infty\}$、みかけの特異点の集合を $\Lambda(t)=\{\lambda_{1}(t),\cdots,\lambda_{g}(t)\}$ とする。みかけの特異点の個数は $g=K(n,m)$ 個であるとする。
また、特異点を取り除いた空間を $\mathbf{Y}(t)=\mathbb{P}^{1}(\mathbb{C})\setminus(\Xi(t)\cup\Lambda(t))$ とし、各特異点の位置は $\xi_{j}(t_{0})=\xi_{j}, \lambda_{j}(t_{0})=\lambda_{j}$ を満たすとする。

\begin{Q}[【核心】係数 $p_j(x,t)$ が $t$ について解析的であるという仮定から、特異点の位置 $\xi_i(t)$ や $\lambda_k(t)$ も $t$ について解析的に動くと導けるか？]
\textbf{A.} 導ける。特異点は係数関数（有理関数）の分母の零点として定義されるため、陰関数定理より、特異点同士が衝突しない（すなわち極の位数が変化しない）限り、係数が $t$ について解析的であればその零点である特異点の位置も $t$ について自動的に解析的な関数となる。
\end{Q}

さて、各 $t$ に対して、(P1)-(P3) が常に成り立つとし、さらに以下の極めて重要な仮定をおく。
\begin{itemize}
    \item[] \textbf{(H):} モノドロミー表現 $\hat{\rho}(t)$ はパラメータ $t$ に依存しない。
\end{itemize}

\begin{Q}[【重要】条件(H)で「モノドロミー表現が $t$ に依存しない」とあるが、これは「微分方程式の解そのものが $t$ に依存しない」ということか？]
\textbf{A.} 違う。解の基本系 $\vec{\varphi}(x, t)$ 自体はパラメータ $t$ に応じて形を変える（依存する）。しかし、特異点を回ったときに解が受ける「線形変換の規則（回路行列 $\Gamma$）」だけは、時間が経っても絶対に変わらない、という意味である。すなわち、解を「$x$ について解析接続する操作」と「$t$ について微分する操作」が完全に可換になるような、特別な時間発展をしている状態を指す。
\end{Q}

\begin{Q}[【核心】モノドロミーを一定に保つという条件(H)は、係数 $p_j(x,t)$ や特異点 $\xi_i(t), \lambda_k(t)$ の動きにどのような「強い制約」をもたらすのか？]
\textbf{A.} 係数関数が「完全積分可能条件（フロベニウスの定理における両立条件）」を満たすことを強制する。
この強烈な制約の下では、本質的な特異点 $\xi_i(t)$ を勝手に動かす（あるいは $\xi_i$ 自身を独立な変形パラメータ $t_i$ とみなす）と、モノドロミーを不変に保つための「つじつま合わせ」として、みかけの特異点 $\lambda_k(t)$ の位置はもはや自由には動けず、ある非線形な微分方程式に従って決定論的に動かざるを得なくなる。この $\lambda_k(t)$ の運動を記述する方程式こそが、パンルヴェ方程式の正体である。
\end{Q}

\begin{definition}[モノドロミー保存変形]
    条件(H)を満足するような方程式の族 \eqref{eq:monodromy_deformation} が存在する時、\eqref{eq:monodromy_deformation} を元の微分方程式 \eqref{eq:linear-ode-n} の \textbf{モノドロミー保存変形 (monodromy preserving deformation)} または \textbf{ホロノミック変形 (holonomic deformation)} という。
\end{definition}
%#endregion
%#region
\begin{definition}[フックスの問題（Fuchs' Problem）]
    連立微分方程式系 \eqref{eq:deformation_system} に関する以下の構成問題を \textbf{フックスの問題} という。

    \textbf{【問題】} \\
    連立系 \eqref{eq:deformation_system} が、元の空間方向の微分方程式 \eqref{eq:monodromy_deformation} のホロノミック変形を与えるための、係数に関する具体的な条件は何か？

    この問題は、解析学的に厳密には次のように言い換えられる。

    \textbf{【問題（厳密版）】} \\
    連立系 \eqref{eq:deformation_system} の解の基本系 $\vec{\varphi}(x,t)$ であって、任意のループ $[\gamma] \in \pi_{1}(x_{0}; \mathbf{Y}(t))$ に対するモノドロミー行列 $\Gamma(\gamma)$ がパラメータ $t$ に一切依存しないようなものが存在するための、\textbf{必要十分条件}を求めること。
    
    （ただし、$\mathbf{Y}(t) = \mathbb{P}^{1}(\mathbb{C}) \setminus (\Xi(t) \cup \Lambda(t))$ は、時刻 $t$ における微分方程式のすべての特異点を除いた正則域である。）
\end{definition}

\begin{Q}[【核心】フックスの問題を解いて得られる「必要十分条件」とは、具体的にどのような形で我々の目の前に現れるのか？]
\textbf{A.} 空間方向の係数 $p_j(x,t)$ と時間方向の係数 $a_j^{(l)}(x,t)$ が満たすべき\textbf{「非線形偏微分方程式系」}として現れる。
この条件は、微分作用素の可換性 $[\partial_x, \partial_{t_l}] = 0$（フロベニウスの完全積分可能条件）を愚直に計算することで得られる。これを解きほぐすことで、$p_j(x,t)$（特にその中に含まれる「みかけの特異点」の座標）が、時間 $t$ に対してどのように時間発展（依存）しなければならないかという具体的なダイナミクスが完全に決定されるのである。
\end{Q}

\begin{Q}[【歴史・展望】なぜこの「フックスの問題」が、パンルヴェ方程式の歴史において決定的な意味を持つのか？]
\textbf{A.} リヒャルト・フックス（R. Fuchs, ラザルス・フックスの息子）が1905年にこの問題を $n=2, m=3$（特異点が4つの2階方程式）という最も簡単な非自明ケースで解いた結果、みかけの特異点 $\lambda(t)$ が満たすべき非線形微分方程式として\textbf{「パンルヴェVI型方程式」}が突如として姿を現したからである。
これにより、元々は全く別の文脈（動く分岐点を持たない方程式の分類）で発見されたパンルヴェ方程式が、実は「等モノドロミー変形」という極めて深い代数幾何学的な起源を持つことが証明されたのである。
\end{Q}
%#endregion