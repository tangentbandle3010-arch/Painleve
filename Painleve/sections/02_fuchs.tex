...
解が多価関数となる場合（すなわち $s \notin \mathbb{Z}$）であっても、適当な複素数 $s$ によって \eqref{eq:gauge_transform} の変換を行い、特性指数を全て整数にでき、かつ非対数的となるような特異点のことを、広い意味で \textbf{みかけの特異点} と呼ぶ。
\end{definition}

さて、$x=\lambda$ がみかけの特異点であるならば、変換 \eqref{eq:gauge_transform} で適切に $s$ を選ぶことで、すべての特性指数を「非負の整数」に揃えることができる。このとき、微分方程式の任意の解は $x=\lambda$ の近傍で正則（テイラー展開可能）となる。
