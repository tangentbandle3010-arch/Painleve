...
\textbf{A.} 複素関数論における「孤立特異点の分類定理（リーマンの定理）」が背後で強力に効いているためである。証明の核心は以下の通り。

\begin{itemize}
    \item \textbf{【$\Rightarrow$ の証明（ODE確定 $\implies$ 解析的確定）】}\\
    ODEの定義より、1価関数 $\vec{z}(x)$ は $x=\xi$ で高々極しか持たない（ローラン展開の負べきが有限個）。よって $|\vec{z}(x)| \le C|u|^{-N}$ と多項式オーダーで評価できる。一方、多価部分 $\Phi(x)$ は $u^\alpha$ や $\log u$ の成分からなるが、これらも例題\ref{ex:regular_singular_points}で見たように多項式オーダー $|u|^{-M}$ で抑えられる。よって解 $\vec{\phi}(x) = \vec{z}(x)\Phi(x)$ 全体も多項式オーダーで抑えられ、解析的な意味で確定特異点となる。
    
    \item \textbf{【$\Leftarrow$ の証明（解析的確定 $\implies$ ODE確定）】}\\
    解 $\vec{\phi}(x)$ が $|u|^{-A}$ で抑えられると仮定する。命題\ref{prop:single_valued_z}より $\vec{z}(x) = \vec{\phi}(x)\Phi(x)^{-1}$ であるが、$\Phi(x)^{-1}$ も $u^{-\alpha}$ などの成分からなるため多項式オーダーで抑えられる。したがって、1価関数 $\vec{z}(x)$ もある多項式オーダー $|u|^{-B}$ で抑えられる。\\
    ここで複素関数論の基本定理より、\textbf{「ある点の近傍で多項式オーダーでしか発散しない1価の解析関数は、その点に高々極しか持たない（真性特異点にはならない）」}ことが知られている。すなわち、$\vec{z}(x)$ のローラン展開は有限の負べきで必ず打ち切られる。これはまさにODEの確定特異点の定義そのものである。
\end{itemize}
\end{Q}
